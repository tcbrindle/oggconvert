...
"""
    ...
    2007-09-25 Small fixes by Alexander Kabakov (ak.anapa@gmail.com)
"""
...
def scan_glade_file(filename):
    ...
    for element in doc.getElementsByTagName('property'):
        if element.getAttribute('translatable') == 'yes':
            ...
            #then element.firstChild==None, lets skip it
            if not (element.firstChild is None):
...
def run():
    ...
    for string, occurrences in scan_tree(basepath):
        ...
        print 'msgid %s' % quote(string.encode('utf-8'))
        print 'msgstr ""'
